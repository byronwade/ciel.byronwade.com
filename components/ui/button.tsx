...
function Button({
  ...
  // Support both the `asChild` pattern (wraps a single child element) and Base
  // UI's `render` prop. When the underlying element isn't a native <button>
  // (e.g. a Next.js <Link> that renders an <a>), Base UI needs
  // `nativeButton={false}` to keep correct semantics.
  const childRender =
    asChild && React.isValidElement(children)
      ? (children as React.ReactElement)
      : undefined
  const resolvedRender = render ?? childRender
